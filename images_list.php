<?php
	...
	if($submit) {
		...
		if($error_code) {
			...
			$numbers = arrayVal($data, 'numbers');
			$names = arrayVal($data, 'names');
			$extensions = arrayVal($data, 'extensions');
			$primaries = arrayVal($data, 'primaries');
		}
	}
?>
	...
	<div style="width:760px;background-color:#fff;float:left;border-bottom: 1px solid #EFEFEF;" align="center">
		<table border="1" >
			...
			<? foreach($numbers as $index => $number) {
				...
				$primary = arrayVal($primaries, $index);
				$extension = arrayVal($extensions, $index);

				$image_content = getVideoImage($name, $type);
				if($image_content['error_code']) {
					$error_code = arrayVal($image_content, 'error_code');
					$image_tag = 'Could not get image.  Error code '.$error_code.': '.getErrorMessage($error_code).', Response:'.arrayVal($image_content, 'response');
				}
				else {
					$image_file = 'cache/' . $number . "." . $extension;

					$data = arrayVal($image_content, 'data');
					file_put_contents($image_file, $data);

					$info = @getimagesize($image_file);

					if($info === false) {
						$image_tag = 'Invalid image data: '.$data;
						unlink($image_file);
					}
					else {
						$image_tag = '<img src="' . $image_file . '"/>';
					}
				}
			?>
				<tr>
